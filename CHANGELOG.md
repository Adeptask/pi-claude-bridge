...
- **Add: models appear automatically from pi-ai's catalog** — the hardcoded 9-model allowlist is gone. The picker is driven by pi-ai's Anthropic catalog: a model Anthropic ships reaches `/model` as soon as pi-ai lists it, with no bridge change. Dated snapshot ids (`claude-opus-4-5-20251101`-style) stay hidden.
- **Fix: 1M policy is measured, not inferred** — the `[1m]` id (needed for 1M through the SDK) is now sent only for models measured to serve it on every plan (Fable, Opus 5/4.8/4.7, Sonnet 5); an unentitled `[1m]` is rejected outright and would fail every turn. Unmeasured models — newly surfaced ones included — serve the safe default, bare id at 200K, until measured and added. Opus 4.7 now sends `[1m]` (previously bare; both measured 1M). Opus 4.5 and Sonnet 4.5 are newly selectable at 200K. Opus 4.6/Sonnet 4.6 keep their measured plan-gated 1M policy; an id proven optimistic can also be pinned per-user with `provider.forceTwoHundredK`. Model selection now prefers an exact id match before partial matches (so `claude-fable-5` no longer resolves to `claude-fable-5-1`) and picks the newest partial instead of relying on list order.
- **Add: Claude Fable 5.1** — `claude-bridge/claude-fable-5-1`, selectable via the `fable` shortcut with 1M context, straight from pi-ai's native catalog entry. Requires Claude Code 2.1.251 or newer, so `@anthropic-ai/claude-agent-sdk` is bumped to `^0.3.267`. `@anthropic-ai/sdk` moves to `^0.124.0` per the new SDK's peer range (types only). Requires pi-ai ≥0.85.0 (pi-coding-agent and pi-tui dev peers track the same release).
