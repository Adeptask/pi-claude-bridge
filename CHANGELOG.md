...
- **Add: models appear automatically from pi-ai's catalog** — the hardcoded 9-model allowlist is gone. The picker and runtime policy (`[1m]` id vs bare, registered context window) now derive from pi-ai's Anthropic catalog: a model Anthropic ships reaches `/model` as soon as pi-ai lists it, with no bridge change. Dated snapshot ids (`claude-opus-4-5-20251101`-style) stay hidden and are sorted so shortcuts (`opus`, `fable`, `sonnet`, `haiku`) keep resolving to the newest member of each family. Opus 4.7 now sends `[1m]` (previously bare; both measured at 1M). Opus 4.5 and Sonnet 4.5 are newly surfaced. Opus 4.6/Sonnet 4.6 keep their measured plan-gated 1M policy; if a future model's declared 1M turns out not to be served, pin it with `provider.forceTwoHundredK`.
