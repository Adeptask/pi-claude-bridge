...
- **Fix: keep the bridge discoverable in worker runtimes when AskClaude is disabled** — add a status-only `claude_bridge_probe` tool so tool-derived worker extension allowlists can load the provider without enabling delegation or adding credentials.

- **Fix: share continuation state across module evaluations** — use one registry so worker shutdown removes its Pi session without leaving stale state.

- **Fix: register the bridge provider in every Pi runtime** — reuse one callback owner while registering the provider for each worker runtime. Runtime settings are captured by that owner, each Pi session keeps its own Claude continuation, and concurrent tool results stay with their query.
